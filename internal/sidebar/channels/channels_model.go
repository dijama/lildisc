...
func fetchSortedChannels(state *gtkcord.State, guildID discord.GuildID, parentID discord.ChannelID) []discord.Channel {
	...
	// mod: hiddenchannels — append hidden channels (with lock icon) after visible ones.
	if hidden := mods.FetchAllChannels(state, guildID, parentID, gtkcord.AllowedChannelTypes); len(hidden) > 0 {

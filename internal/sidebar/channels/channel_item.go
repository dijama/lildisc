...
func bindChannelItem(state channelItemState, item *gtk.ListItem, row *gtk.TreeListRow) func() {
	...
	// mod: channelmenu — right-click context menu on channels
	mods.AttachChannelContextMenu(state.state.Context(), i.child.Box, i.chID)
...
func (i *channelItem) Invalidate() {
	...
	ch, _ := i.state.Offline().Channel(i.chID)
	if ch == nil {
		// mod: hiddenchannels — try cabinet directly for hidden channels
		ch, _ = i.state.Cabinet.Channel(i.chID)
	}
	if ch == nil {
		i.child.content = newUnknownChannelItem(i.chID.String())
		i.item.SetSelectable(false)
	} else if mods.IsChannelHidden(i.state, i.chID) {
		// mod: hiddenchannels — render with lock icon, greyed out
		i.child.content = newHiddenChannelItem(ch)
		i.item.SetSelectable(false)
	} else {
		switch ch.Type {
		case
...
// mod: hiddenchannels — render a locked channel with lock icon overlay.
func newHiddenChannelItem(ch *discord.Channel) gtk.Widgetter {
	icon := NewChannelIcon(ch, func(t discord.ChannelType) (string, bool) {
		return "changes-prevent-symbolic", true // lock icon
	})
	...
	box := gtk.NewBox(gtk.OrientationHorizontal, 0)
	box.AddCSSClass("channel-item")
	box.AddCSSClass("channel-item-hidden")
	box.Append(icon)
	box.Append(label)
